...
> 兩個豁免疊在一起就是整套檢查最薄的一塊。
>
> **【2026-09-22 訂正：原本的量化是錯的，而且錯的方向讓洞是開著的】**
> 這裡原本寫「要把 Δx 從 +12 拉到負值需要 **≥36%** 的遮罩膨脹，而第 5 點在 **≥25%**
> 就觸發 —— 中間有餘裕」。實際重現：把格 3 的眼窗換成看右的內容、在眼窗最左端塗
> 8 欄（183 px）容差內的虹膜色，質心由 +12.00 → **−7.99**，遮罩比率 **1.202**
> —— 只要 **20.2%** 膨脹，而它 **< 上限 1.25**，於是 A–H **全部綠燈**。
> （複審在另一組 fixture 上量到 10.8% 就夠。）攻擊窗正好開在 1.20 與 1.25 之間。
>
> 更關鍵的是**防偽造與合法變化在這個量上耦合，一個數字做不到兩件事**：
> - 要擋住污染攻擊 → 上限必須 **< 1.20**
> - 往下看時虹膜從上眼瞼下滑出來、可見面積**合法地變大** → 實測 **1.35**
>   （下方那句「可見面積只會被眼瞼遮掉、沒有合法理由變大」的前提是錯的 ——
>   它只在 master frame 的虹膜完全沒被遮住時成立，而那不是動畫的畫法。）
>
> 所以改成：**遮罩大小比率降為 warn**（門檻本來就未經校準），硬失敗改由**形狀**承接。
> 虹膜是兩塊近圓的盤（乾淨素材實測：2 塊、各 454 px、長寬比 1.0），
> 而污染是眼窗邊緣的一條長條（183 px、長寬比 **9.4**）。
> 兩者在面積上分不開，在形狀上分得很開，而且**形狀對合法的面積變化免疫**：
> - `SP-7.3/眼窗雜塊`（硬失敗）：兩隻眼睛以外的虹膜色區域總面積 > 較小那隻眼睛的 10%
> - `SP-7.3/虹膜不成形`（硬失敗）：任一隻眼睛的 bbox 長寬比 > 3
>
> selftest 掃過 6–24 欄的**整個攻擊區間**而不只是那一個點 ——
> 只釘單一欄數的話，下次調參數時攻擊窗會靜悄悄挪到隔壁欄數而斷言照樣綠。
>
...
量測法：臉中軸由剪影對稱軸、眼線與瞳心由虹膜遮罩質心（**見下方二次修訂 —— 質心不是瞳心**）、
...
> **⚠️ 2026-09-22 二次修訂：眼線與瞳心也是「量的不是同一個東西」，同一類問題。**
>
> SP-2.4 把眼線定義為「左右**瞳心**連線」，而本條的量測法寫「虹膜**遮罩質心**的 y 平均」。
> 動畫畫法裡上眼瞼一定蓋住虹膜頂端，於是可見虹膜的質心**系統性地低於瞳心** ——
> 複審在真實畫稿上實測，把瞳心畫在正好 0.380、上眼瞼切掉約 30% 虹膜高的畫稿
> 偏離 `0.0201·S`＝**容差的 5.0 倍**，八個變體無一例外。
> 側髮蓋住一隻眼時更糟：瞳心畫在正好 0.410 的畫稿實測 0.4407·S，**7.6 倍容差** ——
> 畫師得把一個已經正確的瞳孔往旁邊挪 15px 才過得了。
>
> **遮擋深度是畫風選擇不是合規屬性**，所以沒有固定偏移可以校正，也沒有「量得更準」這條路。
> 改成量三件量得到的事：
> - **瞳距**（`right − left`，比對 SP-2.5 的 `0.590 − 0.410 = 0.180`）與
>   **左右瞳心中點對臉中軸的對稱性** —— 兩者對眼瞼遮擋**免疫**（上眼瞼同時切兩眼、
>   而且完全不動 x）。實測：遮住虹膜頂端三成，瞳距變化 `0.00002`；
>   而左瞳右移 8px 會讓瞳距縮 `0.0156·S`＝容差的 3.9 倍。**維持硬失敗。**
> - **兩眼等高**（左右虹膜質心 Y 的差）：左右受到的遮擋相同，差值對遮擋深度免疫。
>   **維持硬失敗** —— 一眼畫高了是真缺陷。
> - **絕對的眼線 Y 與單眼瞳心 X**：降為 **warn**，訊息寫明量的是可見虹膜質心而非瞳心。
>   它們同時吃「遮擋」與「移位」、分不開，當硬失敗只會退掉合規畫稿。
> - 左右可見虹膜面積比 < 0.6（一眼被側髮遮住）時，連 warn 都略過並明說「本輪量不準」，
>   不報一個不可信的數字。先前一側完全量不到虹膜時整段**靜默略過**，現在報 error。
>
