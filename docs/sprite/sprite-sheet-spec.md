...
## 可讀性實測：第一輪結果（2026-09-18）與它推翻的東西

規格把凍結風險押在「虹膜直徑與瞳孔位移」兩個值（SP-2.6 / SP-3.3）。
第一輪 36 題盲測（Windows / Chrome、Grafana dark panel 底色、dpr 1.5）顯示
**那個框架抓錯了自由度**。
...
| 參數組 | iris | dx | dy | dy/dx | 正確率 |
|---|---|---|---|---|---|
| small | 0.050 | 0.016 | 0.013 | 0.81 | **11/12 = 92%** |
| big | 0.085 | 0.030 | 0.025 | 0.83 | 10/12 = 83% |
| spec | 0.065 | 0.022 | 0.018 | 0.82 | 9/12 = 75% |

依尺寸：128px 78%、160px 78%、224px 100%。

### 三個結論

**1. 錯誤全部集中在垂直軸。** 6 個錯誤裡 **6 個丟失垂直分量、只有 1 個丟失水平分量**；
對角答錯時 **5/5 被答成相鄰的純水平**（↘→→ 三次、↗→→、↙→←）。

**2. 三組候選其實是同一個形狀。** 它們的 `dy/dx` 都是 **0.82** ——
規格從頭到尾沒有把垂直位移當成獨立的自由度，三組只是同一比例的三種縮放。
所以同一個弱點三組都有，**這也是為什麼放大沒有用**。

**3. 放大反而更差。** `big`（iris 0.085）83%，`small`（iris 0.050）92%。
機制合理：虹膜越小眼白越多，同樣的位移在視覺上佔比越大。
SP-2.6 若要調整，方向應該是**往下調而非往上調**。

### 一個必須誠實記下的干擾

第一輪的刺激圖在「往下看」時把眼睛開口**壓扁**（高度 × 0.74）。
那可能正在**抵銷**虹膜的下移 —— 開口變矮，虹膜就沒有空間往下跑，兩個線索互相取消。
若是如此，↘ 的 0/3 有一部分是刺激圖的畫法造成的，不是參數本身的問題。
第二輪把它列為獨立變因。

### 凍結狀態

**SP-2.6 與 SP-3.3 不凍結。** 第二輪（128px、30 題）固定 iris 0.055 與 dx 0.020，
只變動兩件第一輪從沒變過的事：`dy/dx` ∈ {0.8, 1.4, 2.0}、
往下看的眼瞼畫法 ∈ {壓扁, 整個開口下移}。並夾入純水平題當對照組 ——
對照組若也開始錯，代表問題不在垂直軸，上表的比較就不成立。

---

## 可讀性實測：第二輪（2026-09-20）—— 機制找到了，但規格該凍的不是比值

第二輪 30 題，固定 iris 0.055 與 dx 0.020，只變動 `dy/dx` ∈ {0.8, 1.4, 2.0}
與往下看的眼瞼畫法 ∈ {壓扁, 整個開口下移}。

| 組合 | 正確 | 不確定 | 平均反應 |
|---|---|---|---|
| **1.4 / 開口下移** | **4/4** | **0** | **1068ms** ← 全對且最快 |
| 0.8 / 壓扁 | 2/4 | 1 | 1166ms |
| 0.8 / 開口下移 | 2/4 | 1 | 1756ms |
| 1.4 / 壓扁 | 1/4 | 2 | 1734ms |
| 2.0 / 壓扁 | 0/4 | **4** | 1466ms |
| 2.0 / 開口下移 | 0/4 | **4** | 1218ms |

`dy/dx = 2.0` 的兩種眼瞼**八題全部答「不確定」** —— 不是猜錯，是根本看不出來。

### 機制：虹膜中心相對眼睛開口下緣的位置

```
                     虹膜中心 - 開口下緣      實測
  0.8 / 開口下移        -0.0119            2/4   虹膜還在開口中央 → 位移看不出來
  1.4 / 開口下移        +0.0001            4/4   虹膜緊貼下緣 → 最像在看下面
  2.0 / 開口下移        +0.0121            0/4   虹膜中心跑出開口外 → 只剩上緣一絲
```

「壓扁」那兩組失敗於另一個原因：開口高度只剩 0.74 倍，虹膜上下都被切，
**看得到的面積本來就少**，位置對了也讀不出來。

**所以最佳點不是某個 `dy/dx` 比值，而是「往下看時虹膜中心落在眼睛開口下緣附近」。**
開口半高一改，最佳比值就跟著改 —— 把 1.4 寫進規格凍結下來，換個眼型就不成立。
**SP-3.3 應改寫為幾何關係，不是數字。**

### 兩處必須誠實記下的

**1. 第一輪的機制推論有一版是錯的。** 初版把「開口下移」的虹膜位移算成相對原眼線，
實際上刺激圖是相對**移動後**的開口中心。錯的版本會得出
「2.0/開口下移 可見 50%」這種與資料直接矛盾的結論。上表是重算後的版本。

**2. 對照組是 5/6，不是先前訂的 100%。** 唯一的錯是 `←` 被答成 `↖` ——
多讀出一個垂直分量，不是水平讀不出來，方向上與「垂直線索變強了」一致。
但通過條件是我自己訂的 100%，這裡如實記下沒有達成。

### 第三輪（最後一輪）：容差

規格改寫成幾何規則之後，唯一還缺的是**畫的人可以差多少** —— 那是發包 brief 必須給的數字。
第三輪 32 題、全部 128px、全部「開口下移」，只掃虹膜中心相對下緣的偏移
∈ {-0.010, -0.006, 0, +0.006, +0.010}，並保留純水平對照組。
連續達到 85% 的那一段即為容差帶，公差取其一半。
**若只有 0 那一格過關，代表這個設計沒有公差可言 —— 該改的是眼型而不是位移。**
