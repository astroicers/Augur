...
- 驗證 `composite(directions[4], reactions[j])` 在該格視窗**之外**與 `directions[4]` 逐位元相同。
  **【2026-09-22 落實】** 先前這條是死碼（程式把合成結果指派成底圖本身再自己比自己，
  恆不成立）。現在實際檢查的是它真正要擋的東西：**產權視窗外 alpha = 0 卻帶非零 RGB 的像素**
  （finding id `SP-7.4/透明像素帶色`）。直通道匯出會把它們清成 0，預乘或某些圖層扁平化不會；
  它們在標準 source-over 下看不見，但換一種合成方式就會浮出來。
