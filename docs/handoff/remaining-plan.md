...
### A3-1 寫 `tools/check-sprite-sheets.mjs`
...
- (b) 拿已在版控的 `assets/a1-augur-calm-cutout.png` 當**解碼器回歸 fixture**，能正確還原 RGBA
...
- (f) 腳本內 `grep -- '--force\|--skip'` 零命中；`grep -n '\bgit\b'` 零命中（見下方禁令）
