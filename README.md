...
> （SP-7.1 ~ SP-7.8，十幾條檢查）今天**一次都不會觸發**。
>
> 2026-09-22 的對抗性複審加獨立查證指出：這批檢查全部只對著
> `tools/lib/syntheticSheet.mjs` 的合成假人校準過，而那是**硬邊、平塗、1-bit alpha**
> 的幾何圖形 —— 1-bit alpha 本身就違反規格自己的 SP-2.14。
> 已知第一批真實素材進來時至少四條會對**合規畫稿**誤殺（描邊估計器的兩條、
> SP-6.6 與 SP-2.14 強制柔邊的衝突、SP-2.12 腮紅矩形與眼窗的重疊），
> 卡點是 fixture 畫不出 `strokeInside` / `fillBleedPx` / `strands` / `alphaRampPx`
> 這些真實畫稿必然有的性質。詳見 `docs/ROADMAP.md`。
>
> **結論：這批 sprite 檢查現在的狀態是「已撰寫」，不是「已驗收」。**
> 第一批素材到貨時要預期它們會紅在錯的地方，先校準再當門檻用。
